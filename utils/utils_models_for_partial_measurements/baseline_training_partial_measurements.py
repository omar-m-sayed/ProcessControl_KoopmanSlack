...
class BaselineTraining:
    def __init__(self, latent_space, real_states, nu, steps, learning_rate, batch_size, data_set,
        ...
        self.NN_encoder = Encoder_base_line(real_states * (states_delay + 1) + nu * control_delay, hidden_encoder,
                                            latent_space)
        ...
        self.multi_step_action_training = self.block_henkel_matrix(self.narx_data_set.nn_training_actions, self.steps)
        self.multi_step_action_batch = self.block_henkel_matrix(self.narx_data_set.nn_batch_actions, self.steps)
        self.states_delay = states_delay
        self.control_delay = control_delay

    def train_step(self):
        ## Reconstruction loss
        self.c = self.inverse(self.NN_encoder(self.narx_data_set.nn_batch_states_henkel),
                              self.narx_data_set.batch_states)
        x_hat = self.NN_encoder(self.narx_data_set.nn_training_states_henkel)
        x_hat = (self.c @ x_hat.T).T
        loss_recon = torch.mean((x_hat - self.narx_data_set.training_states) ** 2)

        ## MS_Loss
        multi_step_state = self.NN_encoder(self.narx_data_set.nn_batch_states_henkel)[:-(self.steps - 1), :]
        combined = torch.hstack([multi_step_state, self.multi_step_action_batch])
        multi_step_label = self.block_henkel_matrix(self.NN_encoder(self.narx_data_set.nn_batch_labels_henkel),
                                                    self.steps)
        self.msModels = self.inverse(combined, multi_step_label)
        multi_step_state = self.NN_encoder(self.narx_data_set.nn_training_states_henkel)[:-(self.steps - 1), :]
        ...
        ss_loss = torch.mean(((ss_hat - self.block_henkel_matrix(self.narx_data_set.training_labels, self.steps)) ** 2))
        #
        training_labels_ss = self.block_henkel_matrix(self.NN_encoder(self.narx_data_set.nn_training_labels_henkel),
                                                      self.steps)
    ...
    ----------
        ...
        a_bar = torch.vstack([a, (0.1 ** 0.5) * I])
    ...
    def get_ms_prediction(self, state, action):
        ...
        state, _, action = self.narx_data_set.get_ms_hankel(state, action, state)
